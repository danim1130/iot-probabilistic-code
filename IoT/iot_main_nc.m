function result = iot_main_nc(energy_constants, base_station, energy, error_epsilon, sample_num, k_nums, N, maxM)
    
    p_succ = 1-error_epsilon;
    result = zeros(sample_num, maxM - N + 1);
    ...
    %for i=1:sample_num
        ...
        for j=1:length(k_nums)
            for m_num=N:maxM
                p_succ_modified = p_succ_message(m_num);
                k_num = k_nums(j);
                rng(rng_saved);
                if (k_num == 1)
                    message_count = runOneHopTest(energy_constants, base_station, energy, p_succ_modified, m_num);
                elseif (k_num == 2)
                    message_count = runTwoHopTest(energy_constants, base_station, energy, p_succ_modified, m_num);
                else
                    message_count = runMultiHopApprxTestLimited(energy_constants, base_station, energy, p_succ_modified, k_num, m_num); 
                end
                result(j, m_num - N + 1) = message_count;
            end
        end
    %end
end

function energyLevelsMultiApprx = runMultiHopApprxTestLimited(energy_constants, base_station, energy, p_succ, hop_number, multiplyMessage)
...
function energyLevelsOneHop = runOneHopTest(energy_constants, base_station, energy, p_succ, multiplyMessage)
    node_count = length(energy_constants);
    node_energy_vector = ones(1, node_count) * energy;
    possible_starting_nodes = 1:1:node_count;
    possible_starting_nodes(base_station) = [];
...
function energyLevelsTwoHop = runTwoHopTest(energy_constants, base_station, energy, p_succ, multiplyMessage)
    node_count = length(energy_constants);
    node_energy_vector = ones(1, node_count) * energy;
    possible_starting_nodes = 1:1:node_count;
    possible_starting_nodes(base_station) = [];
