function result = iot_main(energy_constants, base_station, energy, error_epsilon, sample_num, k_num, N, M)
    p_succ = 1-error_epsilon;
    result = zeros(sample_num, 2);
    
    for i=1:sample_num
        result(i, :) = runMultiHopApprxTestLimited(energy_constants, base_station, energy, p_succ, k_num, N, M);
    end
end

function energyLevelsMultiApprx = runMultiHopApprxTestLimited(energy_constants, base_station, energy, p_succ, hop_number, N, M)
    ...
    energyLevelsMultiApprx = zeros(1,2);
    while 1
        ...
        if hop_number > 0%2
            ...
            if (calcLargestPossibilityLimited(energy_constants, source, base_station, node_energy_vector, hop_number, N, M) < log(p_succ))
                break
            end
            ...
            while (common_energy_apprx_high - common_energy_apprx_low) > energy_diff_abs
                ...
                if calcLargestPossibilityLimited(energy_constants, source, base_station, delta_energy_test, hop_number, N, M) < log(p_succ)
            ...
            [logp, graph_path] = calcLargestPossibilityLimited(energy_constants, source, base_station, delta_energy, hop_number, N, M);
        ...
        for i = 1:length(graph_path)
            node = graph_path(i);
            energy_delta = (node_energy_vector(node) - common_energy_apprx_low) / M;
            ...
            if (rand() > binocdf(N-1, M, message_probability, "upper"))
...
function [logp_max, graph_path] = calcLargestPossibilityLimited(energy_constants, source, base_station, energy, hop_number, N, M)
    logp_matrix = M * -(energy_constants ./ (energy' * ones(1, length(energy))));
    ...
    logp_max = log(binocdf(N - 1, M, exp(-current_shortest_vector(base_station)), "upper"));
