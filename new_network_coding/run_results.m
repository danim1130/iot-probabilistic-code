...
for node_count=node_counts
    ...
    for i=1:random_configuration_num
        ...
        node_energy_constants = -1 * (D.^alpha)*teta*sigma_power_z / N^2;
        ...
        for sample = 1:sample_num
            result_matrix = zeros(1, length(Ms));
            for M=Ms
                tic;
                result_part = iot_main(node_energy_constants, BS, energy, error_epsilon, 1, k_num, N, M);
                toc
                result_matrix(:, Ms == M) = result_part(:, 2);
            end
